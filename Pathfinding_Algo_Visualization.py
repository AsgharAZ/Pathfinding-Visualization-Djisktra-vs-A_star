#Project by Syed Asghar Abbas Zaidi 07201, Muhammad Hasan Khan (07712),Shahjahan (07620) from Habib University
...
k = messagebox.askyesno("yo.", "Welcome to the Visualization of the Pathfinding Algorithm by Asghar, Hasan and Shahjahan \n\nDo you want instructions on how to navigate this code?")
if k == 1:
    k2 = messagebox.askyesno("Baked beans aren't actually baked", msg)
    if k2 == 1: #explaining the algorithm
        ...
        messagebox.showinfo("I like trains.", ms)
...
c = c1 + c2 + c3
chosen = messagebox.askyesno("THE CHOICE?", c)
...
#Initializing the arraw. In this Grid, all the information regarding each box which will show up on the window will be stored.
grid = []
...
#function that sets the individual neighbours for every box
    def set_neighbours(self):
        ...
        if self.x > 0:
             #appending to the neighbours array tthe individual neighbours of the box
...
for i in range(columns):
    arr = []
    for j in range(rows):
        #Box (i,j) = x. We are appending the instances of "x" into the array
        arr.append(Box(i, j))
    grid.append(arr)
...
def main():
    begin_search = False #when true, algorithm starts working. we can worry about that later.
 ...
 #logic to pathfinding algorithm!
        ...
        if begin_search and chosen == 0:
            if len(openSet) > 0:
                ...
                if searching:
                    Tk().wm_withdraw()
                    messagebox.showinfo("WAH NO SOLUTUON", "There seems to be no ANSWER! ;-; Zindagi do be lik dat")
                    searching = False
